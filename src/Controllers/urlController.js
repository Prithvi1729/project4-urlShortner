      ...
      try {
        ...
        if (cachedLongUrlData) {
          return res
            .status(200)
            .send({
              msg: "'provided url already exist in record",
              record: cachedLongUrlData,
            });
        }
        ...
        if (newUrl) {
          res
            .status(201)
            .send({ status: true, mst: "data created", data: newUrl });
        } else {
...
const getUrl = async function (req, res) {
  ...
  if (!isValid(shortCode)) {
    res
      .status(400)
      .send({ status: "failed", message: "please enter valid code" });
  }
  ...
  if (!record) {
    return res
      .status(404)
      .send({ status: false, msg: "document not found with given code" });
  }
